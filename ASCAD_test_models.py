...
def rankhamming(predictions, metadata, real_key, min_trace_idx, max_trace_idx, last_key_bytes_proba, target_byte, simulated_key, ret):
    # Compute the rank
    if len(last_key_bytes_proba) == 0:
        # If this is the first rank we compute, initialize all the estimates to zero
        key_bytes_proba = np.zeros(2 ** (len(predictions[0])-1))
    else:
    ...
    for p in range(0, max_trace_idx-min_trace_idx):
        ...
        for i in range(0, 256):
            ...
            if (simulated_key != 1):
                proba = predictions[p][bin(AES_Sbox[plaintext ^ i]).count("1")]
            else:
                proba = predictions[p][bin(
                    AES_Sbox[plaintext ^ key ^ i]).count("1")]
            if proba != 0:
                key_bytes_proba[i] += np.log(proba)
            else:
                # We do not want an -inf here, put a very small epsilon
                # that correspondis to a power of our min non zero proba
                min_proba_predictions = predictions[p][np.array(
                    predictions[p]) != 0]
                if len(min_proba_predictions) == 0:
                    print(
                        "Error: got a prediction with only zeroes ... this should not happen!")
                    sys.exit(-1)
    ...
    sorted_proba = np.array(
        list(map(lambda a: key_bytes_proba[a], key_bytes_proba.argsort()[::-1])))
    real_key_rank = np.where(
        sorted_proba == key_bytes_proba[real_key])[0][0]
    return (real_key_rank, key_bytes_proba)


def rankhamming1(predictions, metadata, real_key, min_trace_idx, max_trace_idx, last_key_bytes_proba, target_byte, simulated_key, ret):
    # Compute the rank
    if len(last_key_bytes_proba) == 0:
        # If this is the first rank we compute, initialize all the estimates to zero
        key_bytes_proba = np.zeros(len(predictions[0]))
    else:
        # This is not the first rank we compute: we optimize things by using the
        # previous computations to save time!
        key_bytes_proba = last_key_bytes_proba
        ...
        for i in range(0, 256):
            ...
            if (simulated_key != 1):
                ...
                proba = predictions[p][bin(
                    AES_Sbox[plaintext ^ key ^ i]).count("1")]
            if proba != 0:
                key_bytes_proba[bin(i).count(
                    "1")] += np.log(proba)/ret[bin(i).count("1")]
            else:
                # We do not want an -inf here, put a very small epsilon
                # that correspondis to a power of our min non zero proba
                min_proba_predictions = predictions[p][np.array(
                ...
                if len(min_proba_predictions) == 0:
                    ...
                    sys.exit(-1)
                min_proba = min(min_proba_predictions)
                key_bytes_proba[bin(i).count("1")] += np.log(min_proba**2)
    # Now we find where our real key candidate lies in the estimation.
    # We do this by sorting our estimates and find the rank in the sorted array.
    sorted_proba = np.array(
        list(map(lambda a: key_bytes_proba[a], key_bytes_proba.argsort()[::-1])))
    real_key_rank = np.where(
        sorted_proba == key_bytes_proba[bin(real_key).count("1")])[0][0]
    return (real_key_rank, key_bytes_proba)


def rankIdent(predictions, metadata, real_key, min_trace_idx, max_trace_idx, last_key_bytes_proba, target_byte, simulated_key):
    # Compute the rank
    if len(last_key_bytes_proba) == 0:
        # If this is the first rank we compute, initialize all the estimates to zero
...
def full_ranks(predictions, dataset, metadata, min_trace_idx, max_trace_idx, rank_step, target_byte, simulated_key, hammingWeight):
    ...
    for t, i in zip(index, range(0, len(index))):
        if hammingWeight == False:
            real_key_rank, key_bytes_proba = rankIdent(
                predictions[t-rank_step:t], metadata, real_key, t-rank_step, t, key_bytes_proba, target_byte, simulated_key)
        else:
            real_key_rank, key_bytes_proba = rankhamming(
                predictions[t-rank_step:t], metadata, real_key, t-rank_step, t, key_bytes_proba, target_byte, simulated_key, ret)
        f_ranks[i] = [t - min_trace_idx, real_key_rank]
    return (f_ranks, key_bytes_proba)
...
def proba_dissect_beta(proba_sboxmuladd, proba_beta):
    ...
    for j in range(proba_beta.shape[1]):
        proba_sboxdeadd = proba_sboxmuladd[:, [
            (beta ^ j) for beta in range(256)]]
...
def proba_dissect_alpha(proba_sboxmul, proba_alpha):
    ...
    for j in range(proba_alpha.shape[1]):
        proba_sboxdemul = proba_sboxmul[:, [
            multGF256(alpha, j) for alpha in range(256)]]
...
def multilabel_predict(predictions):
    ...
    for i in range(16):
        predictions_unshuffledsboxmul.append(proba_dissect_beta(
            predictions_unshuffledsboxmuladd[i], predictions_beta))

    print("Computing sbox probas with shuffle...")
    predictions_unshuffledsbox = []
    for i in range(16):
        predictions_unshuffledsbox.append(proba_dissect_alpha(
            predictions_unshuffledsboxmul[i], predictions_alpha))

    predictions_unshuffledsbox_v = np.array(predictions_unshuffledsbox)
    predictions_permind_v = np.array(predictions_permind)
    predictions_unshuffledsbox_v = np.moveaxis(
        predictions_unshuffledsbox_v, [0, 1, 2], [1, 0, 2])
    predictions_permind_v = np.moveaxis(
        predictions_permind_v, [0, 1, 2], [1, 0, 2])
    predictions_sbox = []
    print("Computing sbox probas...")
    for i in range(16):
        predictions_sbox.append(proba_dissect_permind(
            predictions_unshuffledsbox_v, predictions_permind_v, i))
...
def multilabel_without_permind_predict(predictions):
    ...
    for i in range(16):
        predictions_sboxmul.append(proba_dissect_beta(
            predictions_sboxmuladd[i], predictions_beta))

    print("Computing sbox probas...")
    predictions_sbox = []
    for i in range(16):
        predictions_sbox.append(proba_dissect_alpha(
            predictions_sboxmul[i], predictions_alpha))

    return predictions_sbox


# Check a saved model against one of the ASCAD databases Attack traces
def check_model(model_file, ascad_database, num_traces=2000, target_byte=2, multilabel=0, simulated_key=0, save_file="", hammingWeight=False, ascad_loaded=False, ascad_data=None,):
    check_file_exists(model_file)
    check_file_exists(ascad_database)
    # Load profiling and attack data and metadata from the ASCAD database
    if ascad_loaded == False:
        (X_profiling, Y_profiling), (X_attack, Y_attack), (Metadata_profiling,
                                                           Metadata_attack) = load_ascad(ascad_database, load_metadata=True)
    else:
        (X_profiling, Y_profiling), (X_attack, Y_attack), (Metadata_profiling,
                                                           Metadata_attack) = ascad_data
    # Load model
    ...
    if len(input_layer_shape) == 2:
        ...
        print("Error: model input shape length %d is not expected ..." %
              len(input_layer_shape))
        sys.exit(-1)
    # Predict our probabilities
    print("inside: "+str(Y_attack[0]))
    ...
    if (multilabel != 0):
        ...
        for target_byte in range(16):
            ranks_i = full_ranks(predictions_sbox[target_byte], X_attack,
                                 Metadata_attack, 0, num_traces, 25, target_byte,	simulated_key)
            # We plot the results
  ...
  # We test the rank over traces of the Attack dataset, with a step of 10 traces
        (ranks, key_bytes_proba) = full_ranks(predictions_sbox_i, X_attack, Metadata_attack,
                                              0, num_traces, 10, target_byte, simulated_key, hammingWeight)
        # We plot the results
        x = [ranks[i][0] for i in range(0, ranks.shape[0])]
        y_rank = [ranks[i][1] for i in range(0, ranks.shape[0])]

        #last = np.argwhere(np.array(y_rank) > 0).max()
        # if last+100 < num_traces:
        #    last = last+100
        #x = x[:last]
        #y_rank = y_rank[:last]
        plt.clf()
        ...
        # We plot the results-prop
        x = [i for i in range(0, len(key_bytes_proba))]
        y = [key_bytes_proba[i] for i in range(0, len(key_bytes_proba))]
        plt.clf()
        if (simulated_key != 1):
            if hammingWeight == False:
                plt.axvline(Metadata_attack[0]['key'][target_byte], color='r')
            else:
                plt.axvline(
                    Metadata_attack[0]['key'][target_byte], color='r')
        plt.title('Performance of '+model_file+' against '+ascad_database)
        plt.xlabel('Key-Candidate')
        plt.ylabel('Key-Score')
        plt.suptitle("Max Prop at Key:"+str(np.argmax(y)))
        plt.grid(True)
        plt.plot(x, y)
        if (save_file != ""):
            plt.savefig(save_file+"-keyprop.png")
        else:
            plt.show(block=False)
    return (y_rank, y)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        model_file = "ATMEGA_AES_v1\ATM_AES_v1_fixed_key\ASCAD_data\ASCAD_trained_models\ident\sync\mlp\mlp-ident8.333333333333332.h5"
        ascad_database = traces_file = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_databases/ASCAD.h5"
        num_traces = 1000
        target_byte = 2
        multilabel = 0
        simulated_key = 0
        save_file = "demo_mlp200.png"
        hammingWeight = False
    else:
        # get parameters from user input
        model_file, ascad_database, num_traces, target_byte, multilabel, simulated_key, save_file = read_parameters_from_file(
            sys.argv[1])

    # check model
    check_model(model_file, ascad_database, num_traces,
                target_byte, multilabel, simulated_key, save_file, hammingWeight)
...
def main(ascad_loaded=False, ascad_data=None, num_traces=2000, save_file="results/demo_cnn", hammingWeight=False, model_file="ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/test_cnn.h5", ascad_database="ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_databases/ASCAD.h5",):
    if len(sys.argv) != 2:
        # default parameters values
        #model_file = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/test_cnn.h5"
        #ascad_database = traces_file = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_databases/ASCAD.h5"
        target_byte = 2
        multilabel = 0
        simulated_key = 0
        traces_file = ascad_database
        #save_file = "demo_cnn.png"
        #hammingWeight = False
    ...
    # check model
    (rank, score) = check_model(model_file, ascad_database, num_traces,
                                target_byte, multilabel, simulated_key, save_file, hammingWeight, ascad_loaded, ascad_data)
    return (rank, score)
    # try:
    #input("Press enter to exit ...")
    # except SyntaxError:
