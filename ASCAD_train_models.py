...
def multi_test(input_dim=1400):
    ...
    for i in range(16):
        ...
        x_permind_l.append(permind_branch(x, i))
    model = Model(inputs, [x_alpha, x_beta] +
                  x_sbox_l + x_permind_l, name='test_multi')
    optimizer = Adam()
    model.compile(loss='categorical_crossentropy',
                  optimizer=optimizer, metrics=['accuracy'])
    return model
...
# database
def load_ascad(ascad_database_file, load_metadata=False, loadless=False, length=None):
    ...
    if loadless == True:
        ...
        for index, value in enumerate(X_profiling):
            ...
            segmented_hamming_plot[indexindex].append(
                [value, Y_profiling[index]])
        tmp = len(X_profiling)
        for key in segmented_hamming_plot:
            if len(segmented_hamming_plot[key]) < tmp:
                tmp = len(segmented_hamming_plot[key])
        tmp_profile = list()
        for key in segmented_hamming_plot:
            if length is not None:
                tmp_profile = [*tmp_profile, *
                               segmented_hamming_plot[key][:int(length[key])]]
            else:
                tmp_profile = [*tmp_profile, *
                               segmented_hamming_plot[key][:tmp]]

        tmp1 = np.asarray(tmp_profile)
        tmp2 = np.split(
            tmp1, 2, axis=1)
        X_profiling = np.asarray(np.squeeze(tmp2[0]).tolist())
        Y_profiling = np.asarray(np.squeeze(tmp2[1]).tolist())
...
def multilabel_to_categorical(Y):
    ...
    for i in range(16):
        y['sbox_'+str(i)+'_output'] = to_categorical(Y['sbox_masked']
                                                     [:, i], num_classes=256)
    for i in range(16):
        y['permind_'+str(i)+'_output'] = to_categorical(Y['perm_index']
                                                        [:, i], num_classes=16)
    return y
...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100, multilabel=0, validation_split=0, early_stopping=0, hammingWeight=False, hamming_ready=False):
    ...
    if (multilabel == 1):
        ...
        if hammingWeight == False:
            ...
            if hamming_ready == False:
                for yIndex, item in enumerate(Y_profiling):
                    Y_profiling[yIndex] = bin(item).count("1")
            y = to_categorical(Y_profiling, num_classes=9)
    print(len(y))
    #print(str(y[0])+" | "+str(y[44999]))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        training_model = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/mlp/test_mlp400.h5"
        ...
        #ascad_database = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_databases/ASCAD.h5"
        #training_model = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/mlp_best_ascad_desync0_node200_layernb6_epochs400_classes256_batchsize400.h5"
        #network_type = "cnn2"
        #epochs = 400
        #batch_size = 400
        ...
        ascad_database, training_model, network_type, epochs, batch_size, train_len, validation_split, multilabel, early_stopping = read_parameters_from_file(
            sys.argv[1])

    # load traces
    (X_profiling, Y_profiling), (X_attack, Y_attack) = load_ascad(
        ascad_database, loadless=False)

    if hammingWeight == True:
        classes = 9
    else:
        classes = 256
    ...
    # get network type
    if(network_type == "mlp"):
        best_model = mlp_best(input_dim=len(X_profiling[0]), classes=classes)
    elif(network_type == "cnn"):
        best_model = cnn_best(input_dim=len(X_profiling[0]), classes=classes)
    elif(network_type == "cnn2"):
        best_model = cnn_best2(input_dim=len(X_profiling[0]), classes=classes)
    elif(network_type == "cnn_dense"):
        best_model = cnn_dense(input_dim=len(X_profiling[0]), classes=classes)
    elif(network_type == "cnn_thin"):
        best_model = cnn_thin(input_dim=len(X_profiling[0]), classes=classes)
    ...
    # training
    if (train_len == 0):
        train_model(X_profiling, Y_profiling, best_model, training_model,
                    epochs, batch_size, multilabel, validation_split, early_stopping, hammingWeight)
    else:
        train_model(X_profiling[:train_len], Y_profiling[:train_len], best_model,
                    training_model, epochs, batch_size, multilabel, validation_split, early_stopping, hammingWeight)

    # ASCAD_test_models.main()
    num_traces = 2000
    save_file = "mlpBest_400_variable_400batch"
    ASCAD_test_models.main(num_traces, save_file,
                           hammingWeight, training_model, ascad_database)


def main(ascad_loaded=False, ascad_data=None, hamming_ready=False, training_model="ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/test_cnn.h5", ascad_database="ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_databases/ASCAD.h5", network_type="cnn", hammingWeight=False, num_traces=10000, save_file="demo_cnn.png", length=None, epochs=75, batch_size=200):
    # default parameters values
    # ascad_database = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_databases/ASCAD.h5"
    # MLP training
    # network_type = "cnn"
    # training_model = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/my_mlp_best_desync0_epochs75_batchsize200.h5"
    # training_model = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/test_cnn.h5"
    ...
    if ascad_loaded == False:
        (X_profiling, Y_profiling), (X_attack,
                                     Y_attack) = load_ascad(ascad_database, loadless=True, length=length)
    else:
        (X_profiling, Y_profiling), (X_attack,
    ...
    # get network type
    if(network_type == "mlp"):
        best_model = mlp_best(input_dim=len(
            X_profiling[0]), classes=classes)
    elif(network_type == "cnn"):
        best_model = cnn_best(input_dim=len(
            X_profiling[0]), classes=classes)
    elif(network_type == "cnn2"):
        best_model = cnn_best2(input_dim=len(
            X_profiling[0]), classes=classes)
    elif(network_type == "cnn_dense"):
        best_model = cnn_dense(input_dim=len(X_profiling[0]), classes=classes)
    elif(network_type == "cnn_thin"):
        ...
        best_model = cnn_lstm(input_dim=len(X_profiling[0]), classes=classes)
    elif(network_type == "multi_test"):
        best_model = multi_test(input_dim=len(X_profiling[0]))
    elif(network_type == "multi_resnet"):
        best_model = resnet_v1((15000, 1), 19)
        ...
        sys.exit(-1)
    #  print best_model.summary()
    print(best_model.get_layer(index=0).input_shape[0])

    # training
    if (train_len == 0):
        train_model(X_profiling, Y_profiling, best_model, training_model,
                    epochs, batch_size, multilabel, validation_split, early_stopping, hammingWeight, hamming_ready)
    else:
        train_model(X_profiling[:train_len], Y_profiling[:train_len], best_model,
                    training_model, epochs, batch_size, multilabel, validation_split, early_stopping, hammingWeight, hamming_ready)
